...
def show_feature_tab(
    ...
    if importance_df.empty or importance_df["value"].sum() == 0:
    ...
    top = (
        features_df.sort_values("score", ascending=False)
        .head(20)
    ...
    fig = px.bar(
        ...
        x="score", y="short_title", orientation="h",
    ...
    recency = (
        features_df.sort_values("days_since_last_purchase")
        .head(30)
        .merge(products[["parent_asin", "title"]], on="parent_asin", how="left")
    ...
    st.dataframe(
        top[[
            "parent_asin", "title", "purchase_count",
            "unique_users", "avg_rating",
            "purchase_frequency", "days_since_last_purchase", "score",
        ]],
